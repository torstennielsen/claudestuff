...
(function () {
  ...
  const CSS = `
    .del-artikel-knapper { display: inline-flex; gap: 8px; align-items: center; }
    .del-artikel-ikon { display: inline-flex; align-items: center; justify-content: center; width: 42px; height: 42px; border-radius: 50%; border: none; cursor: pointer; color: #fff; background: #222; transition: background 0.18s ease, transform 0.12s ease; position: relative; }
    .del-artikel-ikon svg { width: 18px; height: 18px; fill: currentColor; display: block; }
    .del-artikel-ikon:hover { transform: scale(1.08); }
    ...
    .del-artikel-ikon--copied { background: #2a7d34 !important; transform: scale(1.08); }
    .del-artikel-ikon::after { content: attr(data-tip); position: absolute; bottom: calc(100% + 7px); left: 50%; transform: translateX(-50%); background: rgba(0,0,0,.82); color: #fff; font-family: inherit; font-size: 12px; line-height: 1; padding: 5px 9px; border-radius: 5px; white-space: nowrap; pointer-events: none; opacity: 0; transition: opacity 0.15s ease; }
    .del-artikel-ikon:hover::after, .del-artikel-ikon:focus-visible::after { opacity: 1; }
  `;
  ...
  function replaceTrigger(el) {
    ...
    wrapper.appendChild(makeBtn('del-artikel-ikon--facebook', ICONS.facebook, 'Del på Facebook', openFacebook));
    wrapper.appendChild(makeBtn('del-artikel-ikon--mail', ICONS.mail, 'Del via e-mail', openMail));
    var copyBtn = makeBtn('del-artikel-ikon--copy', ICONS.copy, 'Kopiér link', function () { copyLink(copyBtn); });
